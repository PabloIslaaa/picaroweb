...
const App = {
    ...
    bindEvents() {
        ...
        // Reload
        UI.els.reloadBtn.addEventListener('click', () => this.handleReload());
        const reloadLeaveBtn = document.getElementById('reload-leave-btn');
        if (reloadLeaveBtn) {
            reloadLeaveBtn.addEventListener('click', () => {
    ...
    startGame() {
        const save = loadSave();
        if (save.totalChips <= 0) {
            this.handleReload();
            return;
        }

        savePreferences(this.selectedDifficulty, this.selectedMode);

        const buyIn = Math.min(1000, save.totalChips);
        ...
        // Deduct buy-in from total
        const newTotal = save.totalChips - buyIn;
        updateChips(newTotal);
    ...
    async endHand() {
        ...
        // Update human chips in storage
        const save = loadSave();
        const newTotal = save.totalChips + this.humanPlayer.chips;
        updateChips(newTotal);
