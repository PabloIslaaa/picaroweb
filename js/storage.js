...
const RELOAD_COOLDOWN_MS = 4 * 60 * 60 * 1000; // 4 hours
...
function canFreeReload() {
    return true;
}

function doFreeReload() {
    const data = loadSave();
    data.totalChips = 1000;
    data.lastFreeReload = Date.now();
    saveSave(data);
    return data;
}
...
function getTimeUntilReload() {
    const data = loadSave();
    const now = Date.now();
    const remaining = RELOAD_COOLDOWN_MS - (now - data.lastFreeReload);
